...
- Toàn bộ nội dung mentor / câu chuyện học viên / số liệu là **dữ liệu thật** đã được duyệt trước khi publish.
- Ảnh story banner gốc (1200×630) được resize xuống `images/stories/thumbs/<slug>.jpg` cho accordion preview (≈14 KB mỗi ảnh). Chạy `.venv/bin/python scripts/build_thumbnails.py` khi đổi ảnh gốc.
- Xem thêm `skills/` để hiểu chi tiết kiến trúc, content recipes, styling, SEO, deployment.
